...
import { FaPaperPlane, FaComments, FaFileAlt, FaClipboardCheck, FaPlus, FaTimes } from 'react-icons/fa';
...
const ChatPage = () => {
  ...
  const [activeForm, setActiveForm] = useState(null); // 'rapport' or 'attachement' or null
      ...
      <div className="chat-main">
        {selectedUser ? (
          <>
            ...
            {/* Modal Form Rapport Journalier */}
            {activeForm === 'rapport' && (
              <div className="card fade-in" style={{ margin: 15, background: '#f0fdf4', border: '2px solid #16a34a' }}>
                ...
                <form onSubmit={handleSendRapport}>
                  ...
                  <div className="grid grid-2">
                    ...
                    <div className="form-group">
                      ...
                      <input type="text" className="input" placeholder="Ex: 5 Ouvriers, 1 Chef de chantier" value={rapportForm.personnels} onChange={(e) => setRapportForm({ ...rapportForm, personnels: e.target.value })} required />
                    </div>
                  </div>
                  <button type="submit" className="btn btn-primary" style={{ width: '100%' }}>Envoyer le Rapport Journalier</button>
                </form>
              </div>
            )}

            {/* Modal Form Attachement */}
            {activeForm === 'attachement' && (
              <div className="card fade-in" style={{ margin: 15, background: '#f0fdf4', border: '2px solid #ca8a04' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                  <h4 style={{ color: '#ca8a04' }}>📊 Nouveau Procès-Verbal d'Attachement</h4>
                  <button className="btn btn-danger" style={{ padding: 4 }} onClick={() => setActiveForm(null)}><FaTimes /></button>
                </div>
                <form onSubmit={handleSendAttachement}>
                  ...
                  <label className="form-label">Lignes d'Attachement des Travaux</label>
                  {attachementForm.articles.map((art, idx) => (
                    <div key={idx} style={{ background: '#fff', padding: 10, borderRadius: 8, marginBottom: 10, border: '1px solid #d1fae5' }}>
                      <input type="text" className="input" placeholder="Désignation de l'article" style={{ marginBottom: 6 }} value={art.designation} onChange={(e) => {
                      ...
                      <div style={{ marginTop: 6, fontSize: 12, fontWeight: 600, color: '#16a34a' }}>
                        Cumul Calculé : {Number(art.qtePrecedente || 0) + Number(art.qteMois || 0)}
                      </div>
                    </div>
                  ))}
                  <button type="button" className="btn btn-outline" style={{ padding: '4px 8px', fontSize: 12, marginBottom: 10 }} onClick={() => setAttachementForm({ ...attachementForm, articles: [...attachementForm.articles, { designation: '', qteContrat: 0, qtePrecedente: 0, qteMois: 0 }] })}>+ Article</button>
                ...
                <div key={msg._id} className={`message ${msg.sender._id === user._id ? 'sent' : 'received'}`} style={{ maxWidth: msg.type !== 'text' ? '85%' : '60%' }}>
                  ...
                  {msg.type === 'rapport_journalier' && (
                    <div style={{ background: '#fff', color: '#111', padding: 12, borderRadius: 10, borderLeft: '5px solid #16a34a' }}>
                      <h4 style={{ color: '#16a34a', margin: '0 0 6px 0' }}>📑 Rapport Journalier du {msg.reportData?.date}</h4>
                      ...
                      <div style={{ marginTop: 6 }}>🚜 <strong>Matériels:</strong> {msg.reportData?.materiels}</div>
                  ...
                  {msg.type === 'attachement' && (
                    <div style={{ background: '#fff', color: '#111', padding: 12, borderRadius: 10, borderLeft: '5px solid #ca8a04', overflowX: 'auto' }}>
                      <h4 style={{ color: '#ca8a04', margin: '0 0 8px 0' }}>📊 Procès-Verbal d'Attachement du {msg.reportData?.date}</h4>
            ...
            <form className="chat-input-area" onSubmit={handleSendText}>
              <input type="text" className="input" placeholder="Écrivez un message textuel..." value={text} onChange={(e) => setText(e.target.value)} />
          ...
          <div className="chat-empty">
            ...
            <p>Sélectionnez un contact pour commencer la communication</p>
          </div>
        )}
      </div>
    </div>
